...
class News_and_stats_acc {
	...
	function _fetch_news()
	{
		...
		$feed = fetch_rss('http:///feeds/rss/cpnews/', 60*60*24*3); // set cache to 3 days
		...
		else
		{
			...
			$ret .= '<div><a onclick="window.open(this.href); return false;" href="'.$this->EE->functions->fetch_site_index().$qm.'URL=http:///blog/'.'">'.$this->EE->lang->line('more_news').'</a></div>';
